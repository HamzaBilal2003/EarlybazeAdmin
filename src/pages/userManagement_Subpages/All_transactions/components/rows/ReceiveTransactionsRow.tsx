...
interface ReceiveTransactionsRowProps {
  displayData: {
    typeimg?: string;
    type?: string;
    asset: string;
...
const ReceiveTransactionsRow: React.FC<ReceiveTransactionsRowProps> = ({
  displayData,
  index,
}) => {
  ...
  return (
    <>
      ...
      {showDetailModel && (
        <TransactionModal
          status={displayData.status === 'successful' ? 'successful' : displayData.status === 'processing' ? 'processing' : 'failed'}
